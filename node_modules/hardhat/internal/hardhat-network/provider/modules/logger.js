...
class ModulesLogger {
    ...
    printIntervalMinedBlockNumber(blockNumber, isEmpty, baseFeePerGas) {
        if (this._emptyIntervalMinedBlocksRangeStart !== undefined) {
            this._print(`Mined empty block range #${this._emptyIntervalMinedBlocksRangeStart} to #${blockNumber}`, { collapseIntervalMinedBlock: true, replaceLastLine: true });
        }
        else {
            this._emptyIntervalMinedBlocksRangeStart = blockNumber;
            if (isEmpty) {
                this._print(`Mined empty block #${blockNumber}${baseFeePerGas !== undefined
                    ? ` with base fee ${baseFeePerGas.toString()}`
                    : ""}`, {
